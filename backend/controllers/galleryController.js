...
const getAllImages = async (req, res) => {
  try {
    const cacheKey = CACHE_KEYS.ALL_GALLERY();
    
    // Try to get from cache first
    const cachedImages = await cacheManager.get(cacheKey);
    if (cachedImages) {
      res.set('X-Cache', 'HIT');
    ...
    // Cache the result
    await cacheManager.set(cacheKey, transformedImages, CACHE_CONFIG.TTL.GALLERY);
...
const getImagesByCategory = async (req, res) => {
  try {
    const cacheKey = CACHE_KEYS.GALLERY_BY_CATEGORY(req.params.category);
    ...
    // Cache the result
    await cacheManager.set(cacheKey, transformedImages, CACHE_CONFIG.TTL.GALLERY_BY_CATEGORY);
...
const getImageById = async (req, res) => {
  try {
    const cacheKey = CACHE_KEYS.GALLERY_BY_ID(req.params.id);
    
    // Try to get from cache first
    const cachedImage = await cacheManager.get(cacheKey);
    if (cachedImage) {
      res.set('X-Cache', 'HIT');
    ...
    if (image) {
      ...
      // Cache the result
      await cacheManager.set(cacheKey, imageObj, CACHE_CONFIG.TTL.GALLERY);
...
const createImage = async (req, res) => {
  try {
    ...
    await cacheManager.invalidatePattern('gallery');
...
const updateImage = async (req, res) => {
  try {
    ...
    if (image) {
      ...
      // Invalidate gallery cache after successful update
      await cacheManager.invalidatePattern('gallery');
...
const deleteImage = async (req, res) => {
  try {
    ...
    if (image) {
      ...
      // Invalidate gallery cache after successful deletion
      await cacheManager.invalidatePattern('gallery');
...
const deleteMultipleImages = async (req, res) => {
  try {
    ...
    // Invalidate gallery cache after successful bulk deletion
    await cacheManager.invalidatePattern('gallery');
